...
tbl1_data$agesqr <- factor(tbl1_data$age^2)
tbl1_data$srvlngsqr <- factor(tbl1_data$srvlng^2)
tbl1_data$genold <- factor(tbl1_data$genold)
tbl1_data <- tbl1_data %>% 
   mutate(genold = fct_recode(genold, "NA" = ""))
tbl1_data <-tbl1_data  %>% 
...
tbl1_data$totchi<- factor(tbl1_data$totchi)
...
```{r}
controlv <- matrix(data = tbl1_data$white, tbl1_data$female,nrow = 1, ncol = 2)
reg1 <- lm(totchi ~  data = tbl1_data)
summary(reg1)
```
