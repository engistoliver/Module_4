...
```{r}
Joined_Data <- Joined_Data %>% select(name, ngirls, anygirls, genold, totchi, party, rgroup, region, age, female)
```
...
```{r}
Joined_Data %<>% mutate(age_squared = age^2)
```

* create an additional variable of the number of children as factor variable
```{r}
Joined_Data %<>% mutate(numberchildren_fact = totchi)
Joined_Data$numberchildren_fact <- as.factor(Joined_Data$numberchildren_fact)
```
