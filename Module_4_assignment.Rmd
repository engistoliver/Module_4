...
is.na(congress.data$genold)

congress.data$party <- as.factor(congress.data$party)

#congress.data$genold %<>% map(as.factor, na.rm=TRUE)
#congress.data$genold[genold != 'B'| genold != G] <- 'NA'
#i dont know how to set missing values to na

#congress.data$genold <- str_pad(congress.data$genold, 0, pad = "NA")

congress.data <- congress.data %>% mutate(party2 = party)

for (i in party2) { if(party2 == '1') { congress.data$party2 <- "D"} else if (congress.data$party2 == '2') {congress.data$party2 <- "R"} else {congress.data$party2 <- "I"}}
#i do not get any sensible out put here, recoding the party 
...
lm.reg <- unlist(lm.reg)

#* Save the main coefficient of interest ($\beta_1$)
beta_1 <- lm.reg$coefficients.genoldG
...
lm.reg_g <- unlist(lm.reg_g)

#* Save the main coefficient of interest ($\beta_1$)
beta_1_g <- lm.reg_g$coefficients.genoldG
...
congress.rep <- filter(congress.data, party==2)

lm.reg_rep <- lm(totchi ~ genold + region +rgroup+age+age_sq+female+white+srvlng+srvlng_sq , congress.rep)
lm.reg_rep <- unlist(lm.reg_rep)
beta_1_rep <- lm.reg_rep$coefficients.genoldG
beta_1_rep

lm.reg_rep_g <- lm(ngirls ~ genold + region +rgroup+age+age_sq+female+white+srvlng+srvlng_sq , congress.rep)
lm.reg_rep_g <- unlist(lm.reg_rep_g)
beta_1_rep_g <- lm.reg_rep_g$coefficients.genoldG
beta_1_rep_g


congress.dem <- filter(congress.data, party==1 | party==3)

lm.reg_dem <- lm(totchi ~ genold + region +rgroup+age+age_sq+female+white+srvlng+srvlng_sq , congress.dem)
lm.reg_dem <- unlist(lm.reg_dem)
beta_1_dem <- lm.reg_dem$coefficients.genoldG
beta_1_dem

lm.reg_dem_g <- lm(ngirls ~ genold + region +rgroup+age+age_sq+female+white+srvlng+srvlng_sq , congress.dem)
lm.reg_dem_g <- unlist(lm.reg_dem_g)
beta_1_dem_g <- lm.reg_dem_g$coefficients.genoldG
beta_1_dem_g

#Where can I find standard errors?
