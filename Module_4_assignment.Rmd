...
reg_totchi_R <- lm(totchi~ genold + age + age_sq + 
                     srvlng + srvlng_sq + rgroup + region + white, combined_dataset, party == "R")
...
reg_ngirls_congress <- lm(ngirls~ genold + age + age_sq + party + srvlng + 
                            srvlng_sq + rgroup + region + white + n_children, combined_dataset)
...
library(stargazer)
stargazer(reg_totchi_congress,reg_ngirls_congress,reg_totchi_D,reg_ngirls_D,reg_totchi_R,reg_ngirls_R,type='text', title="APPENDIX TABLE 1: Evidence on Legislator Child Gender Mix Selection, 108th Congress", align=TRUE,column.labels = c("Congress","Democrats", "Republicans"), column.separate = c(2,2,2), digits=2,keep.stat="n", object.names = TRUE, keep = "genoldG", 
notes = c(
