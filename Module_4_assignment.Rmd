...
```{r, message=FALSE}
# Assigning independents to the democratic party
data[data$party=="I",7] <- "D"

# Democrat subset of observations
data_dem <- data %>%
  filter(party == "D") 
...
```{r, message=FALSE}
# Republican subset of observations
data_rep <- data %>%
  filter(party == "R")
...
coeff_full <- sum_full$coefficients
std_full <- coeff_full[2,2]
