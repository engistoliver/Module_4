...
# The rows in the object data are selected by a logical statement and the column
# selected is the seventh column which the column for party.
...
# data set with observations of democrats. 
...
# Running the regression for Republicans.
...
sum_full_ngirls <- summary(regression_ngirls)
...
coeff_dem <- sum_dem$coefficients
std_dem <- coeff_dem[2,2]

rm(list = c("sum_dem", "coeff_dem"))

regression_dem_ngirls <- lm(ngirls ~ genold + white + female + party + age + 
                            agesq + srvlng + srvlngsq + rgroup + region, data = data_dem)

beta1_dem_ngirls <- regression_dem_ngirls$coefficients["genoldG"]

# Republicans -----------------------------------------------------------------
sum_rep <- summary(regression_rep)
coeff_rep <- sum_rep$coefficients
std_rep <- coeff_rep[2,2]

rm(list = c("sum_rep", "coeff_rep"))

# Setting up the table:
beta1s <- c(beta1_full, beta1_dem, beta1_rep)
stds <- c(std_dem, std_full, std_rep)

table <- rbind(beta1s, stds)
table <- round(table, 2)
rownames(table) <- c("First_child_female", "Standard_errors")
colnames(table) <- c("genoldG_full", "genoldG_dem", "genoldG_rep")
